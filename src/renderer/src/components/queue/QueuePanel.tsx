...
const QueueItem = React.memo(
  function QueueItem({
    ...
    return (
      <div
        className={`interactive-soft group flex items-center gap-2 rounded-xl px-2 py-1.5 ${
      ...
      >
        ...
        <div className="w-6 flex-shrink-0 text-center">
          {isCurrentTrack && isPlaying ? (
            ...
            <span className={`text-xs ${isCurrentTrack ? 'text-orange-500' : 'text-zinc-500 dark:text-zinc-400'}`}>
              {index + 1}
            </span>
          )}
        </div>
        ...
        <div className="flex-shrink-0 text-right">
          <span className="text-xs text-zinc-600 dark:text-zinc-300">{formatTime(track.duration)}</span>
        </div>
        ...
        <button
          ...
          className="interactive-soft focus-ring flex h-6 w-6 flex-shrink-0 items-center justify-center rounded text-zinc-500 opacity-0 transition-all hover:bg-zinc-200/60 hover:text-red-600 group-hover:opacity-100 dark:text-zinc-400 dark:hover:bg-zinc-700 dark:hover:text-red-400"
          title="从队列中移除"
        >
          <DeleteIcon />
        </button>
...
function EmptyQueueState(): React.JSX.Element {
  return (
    <div className="glass-soft m-3 flex flex-col items-center justify-center rounded-2xl py-12 text-center">
      ...
      <p className="text-xs text-zinc-600 dark:text-zinc-300">从音乐库中添加歌曲到队列</p>
    </div>
  )
}
...
const NowPlayingSection = React.memo(function NowPlayingSection(): React.JSX.Element {
  ...
  if (!currentTrack) {
    return (
      <div className="border-b border-zinc-200/70 p-3 dark:border-zinc-700/70">
        <p className="text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">正在播放</p>
        <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-300">没有正在播放的歌曲</p>
      </div>
    )
  }
  ...
  return (
    <div className="border-b border-zinc-200/70 p-3 dark:border-zinc-700/70">
      <p className="mb-2 text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">正在播放</p>
      <div className="flex items-center gap-3">
        <LazyImage
          src={currentTrack.coverUrl}
          alt={currentTrack.album || 'Album art'}
          className="h-14 w-14 flex-shrink-0 rounded-xl object-cover shadow-lg"
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <p className="truncate text-sm font-medium text-orange-600 dark:text-orange-300">{currentTrack.title}</p>
          </div>
          <p className="truncate text-xs text-zinc-600 dark:text-zinc-300">{currentTrack.artist}</p>
          <p className="truncate text-xs text-zinc-500 dark:text-zinc-400">{currentTrack.album}</p>
        </div>
      </div>
    </div>
  )
...
export function QueuePanel(): React.JSX.Element {
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-zinc-200/70 p-3 dark:border-zinc-700/70">
        <h2 className="text-lg font-semibold text-zinc-900 dark:text-zinc-100">播放队列</h2>
        <div className="flex items-center gap-1">
          {queue.length > 0 && (
            <button
              onClick={clearQueue}
              className="interactive-soft focus-ring flex h-8 items-center gap-1.5 rounded-lg px-2 text-xs text-zinc-600 hover:bg-zinc-200/60 hover:text-red-600 dark:text-zinc-300 dark:hover:bg-zinc-800 dark:hover:text-red-400"
              title="清空队列"
            >
              <ClearAllIcon />
              <span>清空</span>
      ...
      <div className="flex-1 overflow-hidden">
        {queue.length === 0 ? (
          ...
          <div className="p-3">
            <p className="mb-2 text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">接下来播放</p>
            <p className="text-sm text-zinc-600 dark:text-zinc-300">队列中没有更多歌曲</p>
          </div>
        ) : (
          <div className="h-full">
            <div className="p-3 pb-1">
              <p className="text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">接下来播放 ({upcomingTracks.length})</p>
            </div>
            ...
            <Virtuoso
              ...
              itemContent={(index) => {
                ...
                return (
                  <div className={`px-2 ${isDragOver ? 'border-t-2 border-orange-500' : ''}`}>
                    <QueueItem
                      track={track}
                      index={actualIndex}
                      isPlaying={isPlaying}
                      ...
                      onDragOver={(e, i) => {
                        ...
                        setDragOverIndex(getActualIndex(i))
                      }}
                      onDragEnd={() => {
                      ...
                      onDrop={(e, i) => {
                        ...
                        const to = getActualIndex(i)
                        if (from !== null && from !== to) reorderQueue(from, to)
                        setDraggedIndex(null)
                        setDragOverIndex(null)
                      }}
                    />
      ...
      {queue.length > 0 && (
        <div className="border-t border-zinc-200/70 p-3 dark:border-zinc-700/70">
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            {queue.length} 首歌曲 · {formatTime(queue.reduce((sum, track) => sum + track.duration, 0))}
          </p>
        </div>
      )}
    </div>
